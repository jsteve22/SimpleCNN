...
from custom_bfv.ntt import NTT
...
class BFV():
	...
	def __init__(self,q=2**15,t=2,n=2**4,std=2,fn=None,h=64,security=128,bitwidth=64,cache_size=1024,block_size=128,cache_hit_rate=0.9):
		...
		self.q = 274877908993
		self.ntt = NTT(n=n,M=0,N=274877908993)
		self.ntt.psi = 124878001
		self.ntt.invpsi = self.ntt.extended_euclidean( self.ntt.psi, self.ntt.N )
	...
	def decrypt(self,ct):
		...
		scaled_pt = self.ntt.iNTT( scaled_pt )
	...
	def plaintext_mult(self, ciphertext, plaintext):
		...
		"""
		plaintext = self.ntt.NTT(plaintext)
